...
- `ts_cities`（`region`列：海外都市のみ`北米`/`南米`/`アジア`/`ヨーロッパ`を設定、国内はnull） / `ts_city_tags`
...
**TOP画面の階層検索（国内/海外→地域→国→都市）とcities.htmlの国・都市プルダウン検索を実装（2026-09-21、ブランチ`claude/top-search-hierarchy-49e9a1`のworktreeで作業）。コード変更あり（`index.html`/`cities.html`）・DB変更あり（`ts_cities`に`region`列追加＋海外主要都市27件を投入）。この変更を今回コミット・push予定。次の宿題：`admin.html`の都市登録フォームに`region`を設定する手段が無い（現状はSQLで直接投入したのみ）ため、管理画面からの新規都市追加時は`region`がnullになりTOPの地域絞り込みに出てこない点に注意（admin.html改修は未着手）。**
...
- **既知の課題（未対応）**: `admin.html`の都市登録・編集フォームに`region`を設定する項目が無い。管理画面から新規に海外都市を追加すると`region`がnullのままになり、TOPの階層検索（海外旅行→地域）に表示されない。将来的に`admin.html`側で国選択時に地域を自動設定する等の対応が必要
