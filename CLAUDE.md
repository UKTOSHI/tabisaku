...
- `ts_cities`（`region`列：海外都市のみ`北米`/`南米`/`アジア`/`ヨーロッパ`を設定、国内はnull／`price_info`・`food_info`・`spots_info`列：物価目安（"2026年9月頃の目安"表記）・おすすめグルメ・観光スポットのテキスト、admin画面のフォームにも項目あり） / `ts_city_tags`
